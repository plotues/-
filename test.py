...
def main(n_digits):
        ...
        n_digits = n_digits
        # 定义滑动框尺寸和灰度图频道数量
        setting = Hog_kmeans(cell_width= 7, cell_height=7, bin_size=8, filenames=r'D:\clustering\training data')
        data_set = setting.get_data_sets()
        #模型训练
        pca = PCA(n_components=n_digits).fit(data_set)#数据降维处理
        kmeans = KMeans(init=pca.components_, n_clusters=n_digits, n_init=1)
        result =kmeans.fit(data_set)
        #保存模型
        jl.dump(value=result, filename=r'D:\clustering\model\model1.pkl')
